...
export const updateSeatByRoom = async (req, res) => {
  try {
    const activeRoom = await showTime.findOne({ roomId: req.params.roomId, date: { $gte: Date.now() } });
    if (activeRoom) return res.status(404).json("Phòng này vẫn đang hoạt động, không thể cập nhật")
